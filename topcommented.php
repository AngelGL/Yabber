...
if ($range_values[$from] > 0) {
	...
	$sql = "SELECT link_id, link_comments as comments FROM links, sub_statuses WHERE id != 1 AND status in ('published','queued') AND date > $from_time AND link_id = link ORDER BY link_comments DESC ";
	$time_link = "date > FROM_UNIXTIME($from_time)";
} else {
	$sql = "SELECT link_id, link_comments as comments FROM links, sub_statuses  WHERE id != 1 AND status in ('published','queued') AND link_id = link ORDER BY link_comments DESC ";
	$time_link = '';
}
