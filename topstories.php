...
if(SitesMgr::my_id()==1){
	 ...
	 $platform_id = "id = ".SitesMgr::my_id()."";
	 }

// Select a month and year
if (!empty($_GET['month']) && !empty($_GET['year']) && ($month = (int) $_GET['month']) > 0 && ($year = (int) $_GET['year'])) {
	$sql = "SELECT SQL_CACHE link_id, link_votes as votes FROM links, sub_statuses WHERE $platform_id AND YEAR(date) = $year AND MONTH(date) = $month AND link = link_id ORDER BY link_votes DESC ";
	...
	if ($range_values[$from] > 0) {
		...
		$sql = "SELECT SQL_CACHE link_id, link_votes-link_negatives as votes FROM links, sub_statuses WHERE $platform_id AND date > $from_time AND status in ('published','queued') AND link_id = link ORDER BY votes DESC ";
		$time_link = "date > $from_time AND";
	} else {
		// Default
		$sql = "SELECT SQL_CACHE link_id, link_votes-link_negatives as votes FROM links, sub_statuses WHERE $platform_id AND status in ('published','queued') AND link = link_id ORDER BY votes DESC ";
		$time_link = '';
	}
}
...
if (!($memcache_key
	...
	$rows = $db->get_var("SELECT count(*) FROM sub_statuses WHERE $platform_id AND $time_link status in ('published','queued') ");
	if ($rows > 0) {
