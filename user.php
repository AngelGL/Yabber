...
function do_profile() {
	...
	$invite_url = urlencode(trim(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, 'whateveryouwant1', $user->username, MCRYPT_MODE_ECB, mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB), MCRYPT_RAND)))));
	$invitations = User::check_invitations_number($user->username);
	$addresses	  = array();
	...
	$vars = compact(
		...
		'entropy', 'percent', 'geo_form', 'addresses', 'friend_icon', 'invite_url', 'invitations'
	);
...
function do_favorites () {
	...
	do_user_subheader(array(_('envíos propios') => get_user_uri($user->username, 'history'), _('votados') => get_user_uri($user->username, 'shaken'), _('suscritos') => get_user_uri($user->username, 'favorites'), _('votados por amigos') => get_user_uri($user->username, 'friends_shaken')), 2,
...
function do_shaken () {
	...
	do_user_subheader(array(_('envíos propios') => get_user_uri($user->username, 'history'), _('votados') => get_user_uri($user->username, 'shaken'), _('suscritos') => get_user_uri($user->username, 'favorites'), _('votados por amigos') => get_user_uri($user->username, 'friends_shaken')), 1,
...
function do_friends_shaken () {
	...
	do_user_subheader(array(_('envíos propios') => get_user_uri($user->username, 'history'), _('votados') => get_user_uri($user->username, 'shaken'), _('suscritos') => get_user_uri($user->username, 'favorites'), _('votados por amigos') => get_user_uri($user->username, 'friends_shaken')), 3);
...
function do_subs() {
	...
	$subs = $db->get_results($sql);
	if ($subs) {
		$title = _('plataformas de') . " $user->username";
