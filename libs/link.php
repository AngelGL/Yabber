...
*******/
	...
	static function sticky($sub_id){
		global $globals;
		$sticky = new Annotation('sticky-post-'.$sub_id);
		if (!$sticky->read() || !$sticky->text) return false;
		$ids = explode(',',$sticky->text);
		if (! $ids) return false;
		$links = array();
		$counter = 0;
		foreach ($ids as $id) {
			...
			if ($link) {
			$link->has_thumb();
			$link->last_page= ceil($link->comments/$globals['comments_page_size']);
			}
			$links[$counter] = $link;
			$counter++;
		}
		...
		global $globals;
		$annotation = new Annotation('sticky-post-'.$this->sub_status_id);
		if (!$annotation->read() || !$annotation->text){ 
			$ids = array($this->id);
		}else{
			$ids = explode(',',$annotation->text);
			if(in_array($this->id, $ids)){
				$clave = array_search($this->id, $ids);
				array_splice($ids, $clave, 1); 
			} else {
				array_push($ids, $this->id);
			}
		}

		$annotation->text=implode(',',$ids);
	...
	function enqueue() {
		...
		if($this->votes == 0 && $this->author == $current_user->user_id && $this->status != 'queued') {
			...
			$this->last_comment = $this->date=time();
			$this->get_uri();
			$db->transaction();
			if( ! $this->store()) {
				$db->rollback();
	...
	function store() {
		...
		// 		if ($to->user_id != $this->author) {
		// 			User::add_notification($to->user_id, 'comment');
		// 			$db->query("insert into conversations (conversation_user_to, conversation_type, conversation_time, conversation_from, conversation_to) values ($to->user_id, 'comment', now(), 'op', $to->id)");
		// 		}
	...
	function store_basic($really_basic = false) {
		...
		if($this->id===0) {
			...
			$r = $db->query("INSERT INTO links (link_author, link_blog, link_status, link_randkey, link_date, link_sent_date, link_last_comment, link_published_date, link_karma, link_anonymous, link_votes_avg, link_content_type, link_ip_int, link_ip) VALUES ($link_author, $link_blog, '$link_status', $link_randkey, FROM_UNIXTIME($link_date), FROM_UNIXTIME($link_sent_date), FROM_UNIXTIME($link_sent_date), FROM_UNIXTIME($link_published_date), $link_karma, $link_anonymous, $link_votes_avg, '$link_content_type', $this->ip_int, '$link_ip')");
	...
	function print_summary($type='full', $karma_best_comment = 0, $show_tags = true) {
		...
		if ($db->get_row("SELECT comment_id from comments where comment_user_id=$current_user->user_id and comment_link_id = $this->id"))
			$this->replied = true;

		if ($globals['now'] - $this->last_comment < 3600){
			$this->recent = true;
		}

		$this->last_page= ceil($this->comments/$globals['comments_page_size']);
...
*/
	...
	function update_comments() {
		...
		$db->query("update links set link_last_comment = now() where link_id = $this->id");
	...
	function is_editable() {
		...
		if($this->sub_mods){
			$mods = unserialize($this->sub_mods);
		} else{
			$mods = array();
		}
		...
		if( $this->author == $current_user->user_id
			...
			|| in_array($current_user->user_id, $mods)
			|| $current_user->admin) {
			return true;
		}
		return false;
	...
	function get_relative_permalink($strict = false) {
		...
		if ( $this->is_sub && ($globals['submnm'] || $strict || self::$original_status || ! $this->allow_main_link) ) {
			if (! empty($globals['submnm']) && $this->sub_status_id == SitesMgr::my_id() && ! $strict && ! self::$original_status) {
				$base = $this->base_url . 'p/'.$globals['submnm'].'/';
			} else {
				$base = $this->base_url . 'p/'.$this->sub_name.'/';
			}
		} else {
