...
function do_header($title, $id='home', $options = false) {
	global $current_user, $dblang, $globals, $db;
	 // if($title != 'Información legal y condiciones de uso | titulares.in' && !$globals['bot']){
	...
	if ($this_site->sub) {
		$this_site->url = $this_site->base_url.'p/'.$this_site->name;
	} else {
	...
	if (! is_array($options)) {
		...
		//$left_options[] = new MenuOption(_('populares'), $globals['base_url'].'top_visited', $id, _('historias más visitadas/leídas'));
		...
		if ($this_site->enabled && empty($this_site_properties['new_disabled']) || $current_user->user_id == $this_site->owner) {
			$left_options[] = new MenuOption(_('nuevo tema'), $globals['base_url'].'submit', $id, _('enviar nuevo tema'));
		}


		$right_options = array();
		$right_options[] = new MenuOption(_('plataformas'), $globals['base_url_general'].'plataformas?all', $id, _('plataformas'));
		$right_options[] = new MenuOption(_('chat'), $globals['base_url'].'chat', $id, _('chat'));
		...
		$right_options[] = new MenuOption(_('plataformas'), $globals['base_url_general'].'plataformas', $id, _('plataformas'));
		$right_options[] = new MenuOption(_('chat'), $globals['base_url'].'chat', $id, _('chat'));
		$right_options[] = new MenuOption(_('plaza'), post_get_base_url(), $id, _('leer o escribir notas y mensajes privados'));
		$right_options[] = new MenuOption(_('galería'), 'javascript:fancybox_gallery(\'all\');', false, _('las imágenes subidas por los usuarios'));
	}
	...
	if($id == 'profile'){
		...
		$user->all_stats();
		$user->history_link= get_user_uri($user->username, 'history');
		$user->comments_link = get_user_uri($user->username, 'commented');
		$user->posts_link = post_get_base_url($user->username);
		$vars = compact('can_modify','user','friend_icon','sourceUrl', 'url','title', 'greeting', 'id', 'left_options', 'right_options', 'sites', 'this_site', 'this_site_properties', 'subs');
	} else {
		$vars = compact('title', 'greeting', 'id', 'left_options', 'right_options', 'sites', 'this_site', 'this_site_properties', 'subs');
	}

	return Haanga::Load('header.html', $vars);
}

function do_header_login($title, $id='home', $options = false) {
	global $current_user, $dblang, $globals, $db;
	...
	if (! is_array($options)) {
		...
		$right_options = array();
		$right_options[] = new MenuOption(_('p/'), $globals['base_url_general'].'subs', $id, _('sub menéames'));
	...
	return Haanga::Load('login_page.html', $vars);
}
...
function print_subs_form($selected = false) {
	...
	if (! empty($globals['submnm']) && !$current_user->admin) {
		$subs = false;
	} else {
...
function do_trends($what=false) {
	...
	$min_date = date("Y-m-d H:i:00", $globals['now'] - 172800*15); // 48 hours (edit! 2zero)
	// $from_where = "FROM links, sub_statuses WHERE id = ".SitesMgr::my_id()." AND link_id = link and link_date > '$min_date' and link_status $status";
	// $max = 3;
	...
	// futuro
	$from_where = "FROM posts WHERE post_date > '$min_date'";
	$max = 3;
	$res = $db->get_col("select post_tags $from_where");
	...
	if ($res) {
		...
		foreach ($words as $word => $count) {
			...
			if ($globals['base_search_url']) {
				...
				$content .= $globals['base_url'].'search?w=posts&amp;q=%23';
			}
			$content .= urlencode($word).'&amp;o=date">#'.$word.'</a></li>';
		}
		if ($max > 2) {
			$vars = compact('content', 'title', 'url');
			$output = Haanga::Load('trending_topic.html', $vars, true);
			echo $output;
		} else {
			$output = ' ';
		}
...
function do_user_widget() {
	global $user, $current_user, $dblang, $globals, $db;
	...
	// 	memcache_madd($key, $output, 300);
	//print_r($user);
	$user->following_link= get_user_uri($user->username, 'friends');
	$user->followers_link = get_user_uri($user->username, 'friend_of');
	$user->posts_link = post_get_base_url($user->username);
	return Haanga::Load('user_widget.html', compact('user', 'friend_icon', 'url', 'sourceUrl', 'isCurrentUser'));
}

function do_user_suggestions() {
	global $dblang, $globals, $db;

	if (! empty($globals['mobile']) || !empty($globals['submnm'])) return;

	$output = '';
	$key = 'user_suggestions_'.$globals['v'];
	if(memcache_mprint($key)) return;

	$min_date = date("Y-m-d H:i:00", $globals['now'] - 86400*4); //24h*4
	$select = "SELECT user_id, user_login, user_avatar, user_names, count(*) as count ";
		$from_where = " FROM posts, users WHERE user_level not in ('disabled', 'autodisabled') and post_date > '$min_date' and post_user_id=user_id GROUP BY post_user_id";
		$order_by = " ORDER BY count DESC ";
	$sql = "$select $from_where $order_by LIMIT 5";
	$entries = $db->get_results($sql);
	//print_r($entries);

	if ($entries) {
		$objects = array();
		$title = _('¿a quién seguir?');
		$url = $globals['base_url'].'rsss';
		foreach ($entries as $entry) {
			$obj = new stdClass();
			$obj->user_id = $entry->user_id;
			$obj->avatar = $entry->user_avatar;
			$obj->names = $entry->user_names;
		...
		$vars = compact('objects', 'title', 'url');
		$output = Haanga::Load('user_suggestions.html', $vars, true);
		echo $output;
		memcache_madd($key, $output, 300);
	}
}
...
function do_active_subs(){
	global $db;
	$output = ' '; // Use a space to be sure it's memcached.

	$key = 'active_subs_front_'.$globals['site_shortname'].$globals['v'];
	if(memcache_mprint($key)) return;
	echo '<!-- Calculating '.__FUNCTION__.' -->';

	$sql = "select subs.*, user_id, user_login, user_avatar, count(*) as c from subs LEFT JOIN users ON (user_id = owner), sub_statuses where date > date_sub(now(), interval 5 day) and subs.id = sub_statuses.id and sub_statuses.id = sub_statuses.origen and sub_statuses.status in ('published','queued') and subs.sub = 1 group by subs.id order by c desc limit 5";
	$subs = $db->get_results($sql);
	if($subs){
		$output = Haanga::Load("active_subs_front.html", compact('subs'), TRUE);
		echo $output;
	}
	memcache_madd($key, $output, 300);
}
...
function do_last_subs($status = 'published', $count = 10, $order = 'date') {
	...
	$ids = $db->get_col("select link from sub_statuses, subs, links where date > date_sub(now(), interval 100 hour) and status in ('queued','published') and sub_statuses.id = origen and subs.id = sub_statuses.id and owner > 0 and not nsfw and link_id = link order by $order desc limit $count");
	if ($ids) {
		$links = array();
		$title = _('Novedades en Plataformas');
		foreach($ids as $id) {
			$link = Link::from_db($id);
			if (! $link) continue;
		...
		$subclass = 'brown';
		$url = $globals['base_url_general'].'plataformas';
...
function do_site_statistics() {
	global $db, $globals, $dblang;

	if ($globals['mobile'] || $globals['submnm']) return;

	$key = "site_statistics_".$globals['v'];
	if(memcache_mprint($key)) return;

	$output = ' ';

	$users = $db->get_results("SELECT (SELECT COUNT(*) FROM users) total_users,(SELECT COUNT(*) FROM links) total_posts, (SELECT COUNT(*) FROM subs) total_platforms");
	$total_users = $users[0]->total_users;
	$total_posts = $users[0]->total_posts;
	$total_platforms = $users[0]->total_platforms;
		$subclass = 'brown';
		$url = $globals['base_url_general'].'subs';
		$vars = compact('subclass', 'total_users', 'total_posts', 'total_platforms');
		$output = Haanga::Load('site_statistics.html', $vars, true);
		echo $output;
...
function do_sub_statistics() {
	global $db, $globals;
	if ($globals['mobile'] || ! $globals['submnm']) return;
	$this_site = SitesMgr::get_info();
	$subscriptors = SitesMgr::get_subscriptors($this_site->id);
	Haanga::Load('sub_statistics.html', array('subscriptors' => $subscriptors, 'this_site' => $this_site));
	return;
}

function do_sub_admin_tools() {
	global $db, $globals, $current_user;
	$this_site = SitesMgr::get_info();
	if ($globals['mobile'] || ! $globals['submnm'] || ($current_user->user_id != $this_site->owner && !$current_user->admin) ) return;
	
	Haanga::Load('sub_admin_tools.html', array('self' => $this_site));
	return;
}

function do_sub_moderators() {
	global $db, $globals;
	if ($globals['mobile'] || ! $globals['submnm']) return;
	$mods_ids = SitesMgr::get_mods();
	if(empty($mods_ids)) return;
	$this_site = SitesMgr::get_info();
	if($this_site->owner != 1) array_push($mods_ids, $this_site->owner);

	$ids = join(',',$mods_ids);
	$ids = "(".$ids.")";
	$sql = "select user_id, user_login, user_avatar from users where user_id in $ids";
	$mods = $db->get_results($sql);
	Haanga::Load('sub_moderators_widget.html', array('mods' => $mods));
	return;
}

function do_apps_widget() {
	global $globals;
	//if ($globals['mobile'] || ! $globals['submnm']) return;
	Haanga::Load('apps_widget.html');
	return;
}
