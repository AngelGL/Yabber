...
class Comment extends LCPBase {
	...
	function prepare_summary_text($length = 0) {
		...
		$this->can_edit =  (! isset($this->basic_summary) || ! $this->basic_summary ) && ( ($this->author == $current_user->user_id && $globals['now'] - $this->date < $globals['comment_edit_time'])  || (($this->author != $current_user->user_id || $this->type == 'admin') && ($current_user->user_level == 'god' || $current_user->user_level == 'admin') ) );
...
//TODO
	...
	static function print_form($link, $rows=5) {
		...
		if($link->date < $globals['now']-$globals['time_enabled_comments'] || $link->comments >= $globals['max_comments'] || ($link->status == 'closed' && $current_user->user_level != 'god' )) {
	...
	static function save_from_post($link, $redirect = true) {
		...
		if (!$current_user->admin && $current_user->user_karma < 5.8) { // Don't check in case of admin comments or higher karma
			...
			if (intval($db->get_var("select count(*) from comments where comment_link_id = $link->id and comment_ip='$comment->ip' and comment_user_id != $comment->author")) > 3) {
		...
		if (mb_strlen($comment->content) < 2 || ! preg_match('/[a-zA-Z+:-]/', $_POST['comment_content'])) { // Check there are at least a valid char
	...
	function update_conversation() {
		...
		foreach ($orders as $order => $val) {
			if ($refs > 50) { // Limit the number of references to avoid abuses/spam
