...
class UserAuth {
	...
	*/
	...
	function Authenticate($username, $pass = false, $remember = false/* Just this session */, $pushId) {
		global $db, $globals;
		...
		if ($user->user_id > 0 && ($pass === false || self::check_hash($user->user_pass, $pass))) {
			...
			if($_SERVER['HTTP_X_REQUESTED_WITH'] == "com.titulares.helloworld") {
    			$db->query("UPDATE users SET user_push_id = '$pushId' WHERE user_id = $user->user_id LIMIT 1");
			}
