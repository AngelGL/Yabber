...
class SitesMgr {
	...
	static public function get_sub_subs($id = false) {
		...
		return $db->get_results("select subs.* from subs, subs_copy where dst = $id and id = src ORDER BY name ASC");
	}
	...
	static function delete_mod($id, $user_id) {
		global $db;
		$current_mods = unserialize($db->get_var("select mods from subs where id = $id"));
		$new_mods = array_diff($current_mods, array($user_id));
		$new_mods_serialize = serialize($new_mods);
		return $db->query("update subs set mods = '$new_mods_serialize' where id = $id");
	}

	static function get_mods($id = false) {
		global $db;
		if ($id == false) {
			$id = self::my_id();
		}
		$current_mods = unserialize($db->get_var("select mods from subs where id = $id"));
		if(!$current_mods) $current_mods=array();
		return $current_mods;
	}
	...
	static public function get_subscriptions($user) {
		...
		return $db->get_results("select subs.*, user_id, user_login, user_avatar from subs LEFT JOIN users ON (user_id = owner), prefs where pref_user_id = $user and pref_key = 'sub_follow' and subs.id = pref_value order by name asc");
	}
	...
	static function get_subscriptors($value) {
		global $db;
		return $db->get_var("SELECT count(*) from prefs where pref_key = 'sub_follow' and pref_value = $value");
	}
