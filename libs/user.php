...
define('FRIEND_YES', '<button class="btn following-button"></button>');
define('FRIEND_BOTH', '<button class="btn following-mutuo-button"></button>');
define('FRIEND_NO', '<button class="btn follow2-button"></button>');
define('FRIEND_OTHER', '<button class="btn follow2-button"></button>');
define('FRIEND_IGNORE', '<button class="btn ignored-button"></button>');
...
class User {
	...
	static function add_notification($id, $type, $value = 1) {
		...
		if (! is_null(User::get_notification($id, $type))) {
			//$user = User::get_user_telegram($id);
				//Telegram::notificacion($user, $type);
			$user_to= User::get_user_telegram($id);
			$user_push = User::get_user_push_id($id);
			$user_from= User::get_username($current_user->user_id);
			if (! is_null($user_to) ){
				Telegram::notificacion($id, $user_to, $type, $user_from);
			}

			if (! is_null($user_push) ){
				OneSignal::notification($id, $user_push, $type, $user_from);
			}
	...
	static function get_user_telegram($id) {
		global $db;
		return $db->get_var("select user_telegram_id from users where user_id = '$id'");
	}

	static function get_user_push_id($id) {
		global $db;
		return $db->get_var("select user_push_id from users where user_id = '$id'");
	}
	...
	static function get_menu_items($view, $user) {
		...
		$items[] = new MenuOption(_('sus plataformas'), get_user_uri($user, 'plataformas'), $id, _('plataformas del usuario'));
		$items[] = new MenuOption(_('temas'), get_user_uri($user, 'history'), $id, _('información de envíos'));
...
// END meta
	...
	/*
		...
		*/
		...
		$user_telegram_id = $db->escape($this->telegram_id);
		$user_push_id = $db->escape($this->push_id);
		if($this->id===0) {
	...
	function all_stats() {
		...
		if ($do_cache && $stats->read()
			...
			$obj->total_following = (int) $db->get_var("select count(*) from friends where friend_from = $this->id");
	...
	static function friend_add_delete($from, $to) {
		...
		switch (self::friend_exists($from, $to)) {
			case 0:
				...
				$user_to= User::get_user_telegram($to);
				if (! is_null($user_to) ){
				Telegram::notificacion($to, $user_to, 'friend', User::get_username($from));
				}
	...
	static function check_invitations_number($user) {
		global $db;
		$invitations = intval($db->get_var("select user_invitations from users where user_login='$user'"));
		return $invitations;
	}

	static function decrease_invitations($user) {
		global $db;
		return $db->query("UPDATE users SET user_invitations = user_invitations - 1 where user_login='$user'");
	}

	static function increase_invitations($user) {
		global $db;
		return $db->query("UPDATE users SET user_invitations = user_invitations + 1 where user_login='$user'");
	}
}
