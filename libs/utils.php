...
function clean_text_with_tags($string, $wrap=0, $replace_nl=true, $maxlength=0) {
	...
	$string = preg_replace_callback('/(?:&lt;|<)(\/{0,1})(\w{1,8})(?:&gt;|>)/', function ($matches) {
			global $globals;
	...
	$string = preg_replace('/<\/(\w{1,8})>( *)<(\1)>/', "$2", $string); // Deletes useless close+open tags
...
function put_emojis_callback($matches) {
	...
	if (!$translations) {
		$translations = array(
			...
			'roto2' => 'roto2.png" alt=":roto2:" title=":roto2:" width="19" height="18"',
			'nusenuse' => 'nusenuse.gif" alt=":nusenuse:" title=":nusenuse:" width="42" height="18"',
			'facepalm' => 'facepalm.png" alt=":facepalm:" title=":facepalm:" width="18" height="18"',
			'mola' => 'mola.png" alt=":mola:" title=":mola:" width="19" height="18"',
			'sisi3' => 'sisi3.gif" alt=":sisi3:" title=":sisi3:" width="18" height="18"',
			'gaydude' => 'gaydude.gif" alt=":gaydude:" title=":gaydude:" width="21" height="24"',
			'psoe' => 'PSOE.png" alt=":psoe:" title=":psoe:" width="18" height="18"',
			'ciudadanos' => 'Ciudadanos.png" alt=":ciudadanos:" title=":ciudadanos:" width="18" height="18"',
			'populares' => 'PartidoPopular.png" alt=":pp:" title=":pp:" width="18" height="18"',
			'izqUnida' => 'IzquierdaUnida.png" alt=":iu:" title=":iu:" width="18" height="18"',
			'podemos' => 'PODEMOS.png" alt=":podemos:" title=":podemos:" width="18" height="18"',
			'upyd' => 'UPYD.png" alt=":upyd:" title=":upyd:" width="18" height="18"',
		);
	}
...
function normalize_smileys($str) {
	...
	$str=preg_replace('/(\s|^):roto2\\:/i', '$1{roto2}', $str);
	$str=preg_replace('/(\s|^):nusenuse:/i', '$1{nusenuse}', $str);
	$str=preg_replace('/(\s|^):facepalm:/i', '$1{facepalm}', $str);
	$str=preg_replace('/(\s|^):mola:/i', '$1{mola}', $str);
	$str=preg_replace('/(\s|^):gaydude:/i', '$1{gaydude}', $str);
	$str=preg_replace('/(\s|^):sisi3\\:/i', '$1{sisi3}', $str);
	$str=preg_replace('/(\s|^):psoe:/i', '$1{psoe}', $str);
	$str=preg_replace('/(\s|^):ciudadanos:/i', '$1{ciudadanos}', $str);
	$str=preg_replace('/(\s|^):populares:/i', '$1{populares}', $str);
	$str=preg_replace('/(\s|^):podemos:/i', '$1{podemos}', $str);
	$str=preg_replace('/(\s|^):izqUnida:/i', '$1{iu}', $str);
	$str=preg_replace('/(\s|^):upyd:/i', '$1{upyd}', $str);
