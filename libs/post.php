...
class Post extends LCPBase {
	...
	function store($full = true) {
		...
		$post_tags = $this->get_hashtags($post_content);
		if($this->id===0) {
			$this->ip = $globals['user_ip_int'];
			$r = $db->query("INSERT INTO posts (post_user_id, post_karma, post_ip_int, post_date, post_randkey, post_src, post_content, post_tags) VALUES ($post_author, $post_karma, $this->ip, FROM_UNIXTIME($post_date), $post_randkey, '$post_src', '$post_content', '$post_tags')");
			...
			$r = $db->query("UPDATE posts set post_user_id=$post_author, post_karma=$post_karma, post_date=FROM_UNIXTIME($post_date), post_randkey=$post_randkey, post_content='$post_content', post_tags='$post_tags' WHERE post_id=$this->id");
			// Insert post_new event into logs
