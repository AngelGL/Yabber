...
$globals['favicon'] = 'img/favicons/favicon-plaza.png';
...
do_header('plaza', _('plaza'), get_posts_menu($tab_option, $user->username));
...
echo '<style> #wrap {background:#f1f1f1;} body {background: #f1f1f1;}</style>';


/*** SIDEBAR RIGHT ****/
echo '<div id="sidebar-notas">';
do_user_suggestions();
do_banner_plaza();
//do_best_stories();
...
/*** SIDEBAR LEFT ****/
echo '<div id="sidebar-notas" style="float:left">';
//do_user_widget();
if ($current_user->user_id > 0 || $user->id != 0) {
do_user_widget();
}
do_banner_plaza();
do_trends('published');
...
if ($current_user->user_id == 0) {
	//print_oauth_icons_notame($_REQUEST['return']);
}
//do_trends('published');
...
echo '<div id="newswrap" style="background: #f1f1f1">'."\n";
...
// }
// echo '<div id="newPost" style="display: block;"><div class="plaza"></div></div>';
echo '<script>window.onload = function() { post_new();}</script>';
