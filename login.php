...
function do_login() {
	...
	// 	echo '<div class="genericform" style="float:left; width:40%; margin: 0; background: none">'."\n";
	// } else {
		echo '<div class="genericform" style="float:auto; background: none">'."\n";
	//}
	...
	if($_POST["processlogin"] == 1) {
		...
		} elseif (strlen($password) > 0 && $current_user->Authenticate($username, $password, $_POST['persistent'], $_POST['pushId']) == false) {
			Log::insert('login_failed', $globals['user_ip_int'], 0);
	...
	echo '<div class="login-logo"></div>';
	echo '<div class="login-title">Ya verás.</div>';
	echo '<input class="login" type="text" name="username" size="25" tabindex="1" id="name" placeholder="usuario o email" value="'.__($username).'" /></p>'."\n";
	echo '<input class="login" type="password" name="password" placeholder="Clave" id="password" size="25" tabindex="2"/></p>'."\n";
	echo '<p><label for="remember" style="color: #fff">'._('recuérdame').': </label><input type="checkbox" name="persistent" id="remember" tabindex="3"/></p>'."\n";
	...
	echo '<p><input type="submit" value="Entrar" class="login" tabindex="4" /></p>'."\n";
	echo '<div class="recoverpass" style="text-align:center"><h5><a href="register">'._('regístrate').'</a></h5><h5><a href="login?op=recover">'._('¿has olvidado la contraseña?').'</a></h5></div>'."\n";
	...
	echo '<input type="hidden" id="myPushId" name="pushId" value=""/>'."\n";
	if($_SERVER['HTTP_X_REQUESTED_WITH'] == "com.titulares.helloworld") {
	echo '<script type="text/javascript">window.onload = function(e){document.getElementById("myPushId").value = Android.getPushId() }</script>'."\n";
	}
...
function do_recover() {
	...
	if (!$sent) {
		...
		echo '<input class="login" type="text" name="email" size="25" tabindex="1" id="name" value="'.htmlspecialchars($_POST['email']).'" />'."\n";
		echo '<p style="color: #fff">'._('(recibirás un e-mail que te permitirá editar tus datos)').'</p>&nbsp;<br/>';
		echo '<input type="hidden" name="recover" value="1"/>'."\n";
		echo '<input type="hidden" name="return" value="'.htmlspecialchars($_REQUEST['return']).'"/>'."\n";
		echo '<div style="max-width: 302px; margin: auto">'."\n";
		ts_print_form();
		echo '</div>'."\n";
