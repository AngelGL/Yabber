<div id="match-{{match.id}}" class="game">
	...
	{% if match.score_local %}
		<h2>{{match.score_local}} - {{match.score_visitor}}</h2>
	{% endif %}
	<h3>{{match.ts_date|date:"d-m-Y"}} a las {{match.ts_date|date:"H:i"}}</h3>
	...
	<div class="teams"><!--to group 2 teams-->
		...
		<div class="team-votes-box-A">
			<div class="team-votes-count">
				<div class="team-votes-number">
					<a class="fancybox">{{match.votes_local}}</div>
				<div class="team-votes-meneos">votos</div>
			</div><!--team-votes-count-->
			{% if current_user.user_id != 0 %}
			<div class="team-votes-button-A"><div class="{% if match.vote!==1%}team-votes-button-B{%endif%}">
				<div class="team-votes-menealo {% if !match.is_votable%}disabled{%endif%} {% if match.vote===1%}team-voted{%endif%} match-{{match.id}}">
					{% if (match.vote == 1) %}
						{% trans _("¡votado!") %}
					{% else %}
						{% if match.is_votable %}
		...
		<div class="team team-votes-box-C">
			...
			<div class="team-votes-button-A"><div class="{% if match.vote!==0%}team-votes-button-B{%endif%}">
				<div class="team-votes-menealo {% if !match.is_votable%}disabled{%endif%} match-{{match.id}}">
					{% if (match.vote === 0) %}
						{% trans _("¡votado!") %}
					{% else %}
						{% if match.is_votable %}
		...
		<div class="team-votes-box-B">
			...
			<div class="team-votes-button-A"><div class="{% if match.vote!==2%}team-votes-button-B{%endif%}">
				<div class="team-votes-menealo {% if !match.is_votable%}disabled{%endif%} {% if match.vote===2%}team-voted{%endif%} match-{{match.id}}">
					{% if (match.vote == 2) %}
						{% trans _("¡votado!") %}
					{% else %}
						{% if match.is_votable %}
