...
if(isset($_POST["phase"])) {
	switch ($_POST["phase"]) {
		case 1:
			do_header(_('nuevo tema'), _('nuevo tema'));
			if (! do_submit1()) {
			...
			break;
		case 2:
			do_header(_('nuevo tema') . " (Vista preliminar)", _('nuevo tema'));
			if (! do_submit2()) {
				// Just to display error messages
				$link = new Link;
				$link->randkey = rand(10000,10000000);
	...
	do_header(_('nuevo tema') . " 2/3", _('nuevo tema'));
	do_submit1();
} else {
	// check_already_sent();
	// do_header(_('nuevo tema') . " 1/3", _('nuevo tema'));
	// do_submit0();
	do_header(_('nuevo tema') . " 2/3", _('nuevo tema'));
	do_submit1();
}
