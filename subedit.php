...
if (! $can_edit) {
	...
	if ($_POST['created_from']) {
		...
		if ($id && empty($errors)) {
			...
			header("Location: ".$globals['base_url_general']."p/$sub->name/subedit");
			die;
		}
		if (! $id) {
...
function save_sub($id, &$errors) {
	...
	$name = mb_substr(clean_input_string($_POST['name']), 0, 15);
	...
	if ($_POST['intro_max_len'] > 10000) $_POST['intro_max_len'] = 10000;
