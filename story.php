...
// $regexhd = '/(\s|^)\+prv\b/';
// if (preg_match($regexhd, $link->title)) {
//     if(!$current_user->admin){
//     	do_error(_('Solo admins...'), 302);
//     }
// }
...
/*** SIDEBAR ****/
echo '<div id="sidebar" style="margin-top:-2px !important">';
do_sub_statistics();
...
function print_story_tabs($option) {
	...
	if (!$globals['bot']) { // Don't show "empty" pages to bots, Google can penalize too
		...
		if ($globals['link']->sent_date > $globals['now'] - 86400*30 && $current_user->admin) { // newer than 30 days
			echo '<li class="'.$active[4].'"><a href="'.$globals['permalink'].'/log">'._('registros'). '</a></li>';
		}
		if ($globals['link']->date > $globals['now'] - $globals['time_enabled_comments'] && $current_user->admin) {
