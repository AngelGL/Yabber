...
function check_chat() {
	...
	if ($current_user->user_id > 0 && mb_strlen(strip_tags($comment)) > 0) {
...
function get_chat() {
	global $db, $events, $last_timestamp, $globals, $max_items, $current_user, $time_f;
	...
	if ($globals['subs']) {
			$room = SitesMgr::my_id();
			$filter = 'and chat_room in ('.$room.',"friends","admin") ';
	} else {
			$filter = 'and chat_room in ("all","friends","admin") ';
	}
	$res = $db->get_results("select * from chats where chat_time > $time_f $filter order by chat_time desc limit $chat_items");
	if (!$res) return;
