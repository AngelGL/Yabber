...
var now_ts = now.getTime();
  ...
  function startRecording(button) {
    ...
    $('#grabar').addClass('animated pulse');
  }
    ...
    recorder.clear();
    $('#grabar').removeClass('animated pulse');
  }
  ...
  function createDownloadLink(button) {
    recorder && recorder.exportWAV(function(blob) {
      ...
      var $textarea;
      var commentObj = $(button).closest('form');
      $('#subiendo').addClass('fa fa-spinner fa-spin');
      ...
      li.appendChild(au);
			...
			$.ajax({
			        url :  base_url + 'backend/clypUpload',
			        type: 'POST',
			        data: data,
			        contentType: false,
			        processData: false,
			        success: function(data) {
			          var caretPos = $textarea[0].selectionStart;
						...
						$("#subiendo").removeClass('fa fa-spinner fa-spin');
				    },
			        error: function() {
			          alert("not so boa!");
			        }
...
function open_menu(){
	var e = document.getElementById("suscripciones");
	e.classList.toggle("inuse");
}
...
function delete_mod_sub(id, user_id) {
	var url = base_url + 'backend/sub_mod_delete';
	$.post(url,
		{ id: id, key: base_key, user: user_id},
		function(data) {
			if (data.error) {
				mDialog.notify("{% trans _('Error:') %}"+data.error, 5);
				return;
			...
			$mod = $('#mod_'+user_id);
			$mod.addClass("on").remove();
		}
	, "json");
	reportAjaxStats('html', "sub_mod_delete");
}
...
function changeTheme(theme){
	if (theme.value=='default'){
		...
		createCookie("theme", theme.value, 0);
	}
	location.reload();
}
...
function show_inresponse(id, users) {
	dom_id = '#pid-'+ id;
	html = 'En respuesta a ';
	users = ["adri","pepe"];
	users.forEach(function(user) {
    	html += '<a href="/backend/get_post_url?id='+user+','+id+';1">'+user+'</a> ';
	});
...
var fancyBox = new function () {
	this.parse = function ($e) {
		...
		if ((v = myHref.match(/(?:youtube\.com\/(?:embed\/|.*v=)|youtu\.be\/)([\w\-_]+).*?(#.+)*$/))) {
			...
			title = '<a target="_blank" href="jajaj'+myHref+'"'+target+'>'+title+'</a>';
			if(myHref=='/backend/get_chat_users.php') title='';
			href = myHref;
			if (is_mobile) {
				width = '100%';
				height = '100%';
...
;(function ($) {
    ...
    function setUp($el, settings) {
        ...
        $.getJSON('https://www.googleapis.com/youtube/v3/videos?id='+id+'&key=YOUR_API_KEY&fields=items(snippet(title))&part=snippet', function (data) {
...
var emojiKey = new function() {
	...
	this.keyboard = function (caller) {
		...
		if(commentObj.find('textarea').length){
			$textarea = commentObj.find('textarea');
		} else {
			$textarea = commentObj.find('input');
		}
...
(function( $ ) {$.fn.getTeam = function() { this.each(function() {var $team = $(this); var id = $team.html(); $.ajax({type: 'GET', url: 'http://yabber.us/backend/get_comunia.php?id='+id, dataType: "html", success: function (html) {$team.replaceWith(html); }}); }) } }(jQuery));
